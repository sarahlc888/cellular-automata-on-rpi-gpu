...
// See the link in the header for registers, the descriptions are all "todo"
// though.
#define HWRNG_CTRL                                                             \
  ((volatile unsigned int *)0x20104000) // Seems 1 to enable, 0 to disable
#define HWRNG_STATUS ((volatile unsigned int *)0x20104004) // Idk how this works
#define HWRNG_DATA                                                             \
  ((volatile unsigned int *)0x20104008) // Provides 32 random bits
#define HWRNG_INT_MASK                                                         \
  ((volatile unsigned int *)0x20104010) // Used to set interrupt

#define RNG_INITIAL_DISCARDS                                                   \
  0x40000 // Borrowed from the linux driver, RNG needs to start up
...
void random_init(void) {
  ...
  *HWRNG_INT_MASK |= 1;                 // enables interrupt
  while (!((*HWRNG_STATUS) >> 24))
    ; // waits until accumulation complete
}
